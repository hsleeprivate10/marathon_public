...
Primary UI font: system-first Korean sans via `ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`. The interface avoids blocking font downloads on the initial route; vendored Noto Sans KR files may remain available for future opt-in surfaces but are not loaded by the production homepage or calendar unless a later design decision accepts the performance cost. No third-party font request is allowed at runtime.
...
- Structure: date number, race content, optional overflow count. A race with `officialSiteUrl` renders as an event link. A race without `officialSiteUrl` renders as non-clickable event content, never as an anchor backed by `applicationUrl`, a schedule site, MarathonGo, an application seed, or an index/detail source URL. Pending homepage rows keep the visible text `공식 홈페이지 확인 중`. At grid widths, event cards show only the title; venue and status stay in the accessible link label when linked and remain visible in the mobile list. All grid-width titles use a single-line ellipsis to prevent narrow cells from breaking Korean words vertically.
- Provenance display: the UI does not expose source-detail URLs. When MarathonGo fills a missing official date or venue, the row still links only to `officialSiteUrl`; source labels may name MarathonGo via `sources` but must not create a MarathonGo href.
