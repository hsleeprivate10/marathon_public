...
### Homepage Navy Hero
- Structure: full-width navy field with a left-aligned eyebrow, one-to-three-line headline, concise supporting copy, primary orange CTA, optional quiet secondary action, and a right-weighted race photograph. The photograph may extend edge-to-edge behind the copy only when the navy scrim keeps text contrast stable.
- Layout: use `--homepage-content-max` and `--homepage-gutter`; desktop copy occupies no more than 52% of the hero and the focal runner remains visible on the right. At narrow widths, copy and actions precede a fixed-aspect image crop without horizontal overflow.
